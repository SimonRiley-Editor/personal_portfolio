...
export default function FloatingPod({ isChatOpen, onClick, onTriggerHacking, onTriggerCinematic }: FloatingPodProps) {
  ...
  const getStageIcon = () => {
    if (!isMounted) return <Cpu size={20} className="text-nier-beige" />;
    switch (userState.stage) {
      case 1: return <Shield size={20} className="text-nier-beige" />;
      case 2: return <Activity size={20} className="text-nier-beige" />;
      case 3: return <Zap size={20} className="text-nier-beige" />;
      case 4: return <AlertCircle size={20} className="text-nier-red" />;
      default: return <Cpu size={20} className="text-nier-beige" />;
    }
  };
          ...
          >
            ...
            <div className="flex items-center justify-between mb-2 opacity-50">
              <div className="flex items-center gap-2">
                <div className={`w-2 h-2 ${userState.stage === 4 ? 'bg-nier-red animate-ping' : 'bg-nier-beige animate-pulse'}`}></div>
            ...
            <div className="mt-3 h-[1px] w-full bg-nier-beige/10 relative overflow-hidden">
              <motion.div 
                className={`absolute inset-0 ${userState.stage === 4 ? 'bg-nier-red' : 'bg-nier-beige'}`}
                initial={{ width: 0 }}
      ...
      <div className="flex flex-col items-center gap-2">
        ...
        >
          ...
          <div className="relative w-full h-full flex items-center justify-center">
            ...
            <div className={`absolute -bottom-2 w-1 h-2 ${isMounted && userState.stage === 4 ? 'bg-nier-red' : 'bg-nier-beige group-hover:bg-nier-red'}`} />
            <div className={`absolute -left-2 w-2 h-1 ${isMounted && userState.stage === 4 ? 'bg-nier-red' : 'bg-nier-beige group-hover:bg-nier-red'}`} />
            <div className={`absolute -right-2 w-2 h-1 ${isMounted && userState.stage === 4 ? 'bg-nier-red' : 'bg-nier-beige group-hover:bg-nier-red'}`} />
          </div>

          {/* Decorative corners */}
          <div className={`absolute -top-1 -left-1 w-2 h-2 border-t-2 border-l-2 ${isMounted && userState.stage === 4 ? 'border-nier-red' : 'border-nier-beige group-hover:border-nier-red'}`}></div>
          <div className={`absolute -bottom-1 -right-1 w-2 h-2 border-b-2 border-r-2 ${isMounted && userState.stage === 4 ? 'border-nier-red' : 'border-nier-beige group-hover:border-nier-red'}`}></div>
          <div className={`absolute -top-1 -right-1 w-2 h-2 border-t-2 border-r-2 ${isMounted && userState.stage === 4 ? 'border-nier-red' : 'border-nier-beige group-hover:border-nier-red'}`}></div>
