...
export default function CinematicIntro({ onComplete }: CinematicIntroProps) {
  ...
  return (
    <div 
      className="fixed inset-0 z-50 bg-nier-gray overflow-hidden font-mono text-white select-none cursor-crosshair"
      tabIndex={0}
      autoFocus
    >
      <canvas
        ...
        <AnimatePresence mode="wait">
          {message && (
            <motion.div
              ...
              className="text-2xl md:text-4xl tracking-[0.3em] font-bold text-center bg-nier-gray/50 px-8 py-4 rounded border border-white/30 backdrop-blur-sm"
        ...
        <AnimatePresence>
          {gameState === 'lost' && (
            <motion.button
              ...
              exit={{ opacity: 0 }}
              onClick={handleRetry}
              className="mt-8 pointer-events-auto px-8 py-3 border border-white text-white hover:bg-white hover:text-black transition-colors tracking-widest uppercase"
            >
              [RETRY]
            </motion.button>
          )}
        ...
        <AnimatePresence>
          {systemLogs.map(log => (
            <motion.div
              ...
              exit={{ opacity: 0, x: -20 }}
              className="text-xs tracking-widest bg-nier-gray/50 px-3 py-1 border-l-2 border-white"
            >
              {log.text}
            </motion.div>
          ))}
